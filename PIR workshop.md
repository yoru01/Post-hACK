...
<h3 align="center">Price Is Right Casino Edition</h3>
...
In this workshop we'll be designing a casino-like decentralized application in which 2 players place their wagers(bets) and are tasked to guess the right number to win back both their wager and their opponents own.

This workshop is for a project that was created during the Umoja 3 Decentralized Hackathon

We assume that you'll go through this workshop in a directory named `~/reach/price-the-right-casino-edition`

Using the terminal command below we will be creating and accessing a new directory.
```bash
...
As a developer before you start writing any line of code to perform any function we'll have to ask ourselves some questions and produce our answers, then we'll write down an algorithm of how we want that application to work and how it solves those problems listed then and only then can we start writing our first lines of code.

For this Dapp we might have to ask ourselves the following questions:
...
```
The questions above would be answered while this workshop proceeds, so sit tight and enjoy.

# Application Logic
We would start by answering the very first question which concerns the logic of our application.
The logic goes like this:
```
Step 1 - The House aka Deployer sets the game parameters  which are the deadline, the wager, the range of the random numbers and the number of trials the players are allowed to have then he deploys the contract
```
```
...
```
In the `index.rsh` file  we would start by creating the `winner` function that we would be using in the game to coompute the outcome of the game using the guesses of both Alice and Bob and the random number as arguments in the function.
```js
"reach 0.1";
...
```
Next we would be defining the players abilities
...
The reason why this is a casino edition is because instead of having just 2 participants in the game we have 3 in which one is the House aka Organizer and the role of the House is to spectate the game, set the rules and also to collect the wagers if none of them win hereby mimicing a casino. 
...
```
- Here the `House` interacts with his functions hereby setting all the parameters of the  game then deploys the contract then waits for the players to attach.
...
- A while loop is used to implement the feature of the game looping for the amount of tirals set by the house and also until either of the players guess the right number.
...
- In this while loop Alice interaxcts with her `getHand` fuction inputing her guess then Bob does the same then their guesses are been assigned to a var at the end of the loop, these mutable variables are used in the condtions of the while loop to  know if theres a winner .  
...
If you followed this workshop from the beginning to this point congratulations on building a random number guessing game with a wager, you can decide to level up your game and write your own guessing game smart contract with extra features, you could also decide to build a  GUI for this smart contract and add it to your portfolio, this would be a great way to start as a web3 developer. 

For more details on how to build out the GUI for this smart contract you can check out my github [repo](https://github.com/yoru01/The-Price-Is-Right-Casino-Edition).